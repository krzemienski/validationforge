# LinkedIn Post — Week 3 Technical Deep-Dive (Thu May 7)

**Channel:** LinkedIn (native long-form) + personal blog (canonical) + cross-post to dev.to.
**Send time:** Thu May 7, 8:30am ET.
**Word count:** ~1,540.
**Visual:** real screenshot of `hooks/block-test-files.js` source code in editor (`creatives/screenshots/hook-source-annotated.png`). Optional: short looping GIF of the hook denying a Write tool call in real time.
**Purpose:** show the work. Position you as the implementation-detail person, not the abstract-principle person. Engineers who finish this post will trust your tooling judgment for consulting engagements.

---

## Title
> The 50-line hook that blocks AI agents from cheating: how ValidationForge's no-mock enforcement actually works
...
→ github.com/krzemienski/validationforge/tree/main/hooks
...
- This is the substance post for the technical audience. Expect higher engagement on the comments than on broader strategic posts.
- Be ready to answer "why JavaScript instead of [language]?" — the answer is "because Claude Code's hook protocol uses subprocess + JSON over stdin, and Node is the lowest-friction way to handle that on every developer's machine. Bash works too if you prefer."
- Be ready to answer "how do you handle the case where I want a test in src/?" — answer: "you don't write a test file there; the hook blocks it. Write the test in `__tests__/` at the repo root. The hook only enforces the no-mock-in-src rule because that's the categorical pattern that breaks. Tests outside src/ are unaffected."
- DO NOT engage with "this is the wrong approach because [strong opinion about TDD]" — those threads never end well on LinkedIn. Acknowledge once, move on.
