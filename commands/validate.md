...
if [ -n "${FLAG_CLEAN:-}" ]; then
  ...
  # Primary: Node.js script with PID-based lock detection and exact audit log format.
  # Fallback: legacy bash script if evidence-clean.js is absent.
  if [ -f "scripts/evidence-clean.js" ]; then
    VF_EVIDENCE_ROOT="$EVIDENCE_DIR" node scripts/evidence-clean.js
  elif [ -f "scripts/evidence-cleanup.sh" ]; then
    bash scripts/evidence-cleanup.sh "$EVIDENCE_DIR" "$RETENTION_DAYS"
  else
    echo "[vf] WARNING: scripts/evidence-clean.js not found — skipping cleanup." >&2
  fi

  # Dry-run preview (no deletion):
  # VF_EVIDENCE_ROOT="$EVIDENCE_DIR" node scripts/evidence-clean.js --dry-run
fi
```
...
| Generate evidence summary dashboard | ✅ Runs — non-blocking on failure | ✅ Runs — non-blocking on failure | ✅ Runs — non-blocking on failure |

> **Note on dashboard generation:** The `scripts/generate-dashboard.sh` step in REPORT runs in **all enforcement levels** (strict, standard, permissive). It is always **non-blocking** — if dashboard generation fails for any reason, the pipeline logs a warning and continues. A missing or failed dashboard MUST NOT cause the validation run to fail or change any journey verdict.
...
6. Invoke `scripts/generate-dashboard.sh` to produce `e2e-evidence/dashboard.md` and `e2e-evidence/dashboard.html` from the final report and evidence inventory. Dashboard generation is **non-blocking** — if the script fails or exits non-zero, log a warning and continue. A dashboard generation failure MUST NOT cause the validation run to fail or change the overall verdict.

```bash
# Run dashboard generation (non-blocking — failures only produce a warning)
if [ -x scripts/generate-dashboard.sh ]; then
  if ! scripts/generate-dashboard.sh "${EVIDENCE_DIR:-e2e-evidence}" 2>&1; then
    echo "[vf] WARNING: dashboard generation failed — continuing without dashboard.md / dashboard.html" >&2
  fi
else
  echo "[vf] WARNING: scripts/generate-dashboard.sh not found or not executable — skipping dashboard generation" >&2
fi
```
...
Final artifacts saved to the `e2e-evidence/` directory:
- `e2e-evidence/report.md` — canonical per-journey PASS/FAIL verdict report written by the `verdict-writer` agent
- `e2e-evidence/dashboard.md` — human-readable summary dashboard (generated by `scripts/generate-dashboard.sh`; non-blocking)
- `e2e-evidence/dashboard.html` — browser-renderable summary dashboard (generated by `scripts/generate-dashboard.sh`; non-blocking)
- Per-journey evidence files saved under `e2e-evidence/` with descriptive names per journey and step
...
- Dashboard generation failures never change the exit verdict — they only emit a warning to stderr
