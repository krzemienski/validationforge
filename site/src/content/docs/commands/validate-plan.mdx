...
Before generating the plan, read the ValidationForge config written by [`/vf-setup`](/commands/vf-setup/).
...
The plan requires explicit approval before execution. Run [`/validate`](/commands/validate/) to execute an approved plan, or modify the plan file directly before executing.
