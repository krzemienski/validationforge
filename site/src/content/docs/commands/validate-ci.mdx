...
> **Note:** In CI mode, a missing `~/.claude/.vf-config.json` is a **warning, not a failure** — defaults apply and the pipeline continues. Run [`/vf-setup`](/commands/vf-setup/) locally and commit the resulting config to avoid this warning in future runs.
