...
description: >
  Systematic multi-phase audit to determine if an application is ready for
  production deployment. Produces an evidence-backed readiness report with
  PASS/FAIL verdicts per category across code quality, security, performance,
  reliability, observability, documentation, and deployment.
---
...
Systematic multi-phase audit to determine if an application is ready for
production deployment. Produces an evidence-backed readiness report with
PASS/FAIL verdicts per category — not a vibe-check, not a checklist of green
ticks without receipts.
...
This skill complements — but does **not** replace — feature-level validation.
Run it **after** all feature validations have passed, not instead of them.
...
Phases 1–3 can run in parallel. Phases 4–6 can run in parallel. Phase 7
depends on 1–6. Phase 8 synthesizes all results.
...
<Aside type="caution" title="Security and Deployment are hard gates">
...
- The `verdict-writer` agent reads phase reports for the overall verdict.
...
This skill is read-only by default — it inspects code, runs diagnostic
commands, and probes endpoints through the same interfaces real clients use.
It never stores credentials in evidence files (note the "Checklist (no
values!)" constraint on environment variable verification) and never bypasses
security controls.
...
<CardGrid>
  <LinkCard
    ...
    description="The core capture + verdict protocol this audit layers on top of."
    href="/skills/functional-validation/"
  />
  <LinkCard
    title="gate-validation-discipline"
    description="Evidence examination discipline applied to each phase report."
    href="/skills/gate-validation-discipline/"
  />
  <LinkCard
    title="e2e-validate"
    description="Run the feature-level pipeline first, then this audit."
    href="/skills/e2e-validate/"
  />
  <LinkCard
    title="preflight"
    description="Preflight the environment before starting the audit — half the blockers disappear."
    href="/skills/preflight/"
  />
  <LinkCard
    title="full-functional-audit"
    description="Feature-level audit counterpart — use both for full coverage."
  ...
  <LinkCard
    ...
    description="Capture a quality baseline before the audit so you can show improvement over time."
