...
Platform validation for browser-based apps. Drives a real browser via Playwright
MCP or Chrome DevTools MCP, captures screenshots + console + network as evidence,
and writes verdicts backed by what the user would actually see.

## When to Use

Pick `web-validation` when:

- The project has `package.json` + a frontend framework (React, Vue, Svelte,
  Next, Nuxt, Remix, Astro, SvelteKit, etc.).
- You need to validate routes, forms, navigation, or responsive layout.
- You have a real dev server running (or are willing to start one).

For the mechanical "drive the browser" recipes see
[`playwright-validation`](/skills/playwright-validation/). For the higher-level
journey orchestration see [`e2e-validate`](/skills/e2e-validate/).
...
Any JavaScript error in the console is a FAIL unless it is a known, documented,
non-blocking issue.
...
<Aside type="tip" title="Describe what you SEE">
  **GOOD:** "Homepage screenshot shows navigation bar with 5 links, hero
  section with headline 'Welcome to AppName', and a grid of 6 feature cards
  below the fold."
...
| CORS errors in console         | API on different origin, missing headers              | Add CORS headers to API server or use a proxy                           |
...
This skill drives a real browser against a real server. It never writes mocks,
never disables security headers or auth, and never bypasses the same origin
policy — validation happens through the interfaces real users use.
...
<CardGrid>
  <LinkCard
    ...
    description="Lower-level Playwright MCP recipes for the same browser automation this skill orchestrates."
    href="/skills/playwright-validation/"
  />
  <LinkCard
  ...
  <LinkCard
    ...
    description="Evidence examination before marking the web validation complete."
    href="/skills/gate-validation-discipline/"
  />
  <LinkCard
    title="e2e-validate"
    description="Multi-step journey orchestration that wraps web-validation."
    href="/skills/e2e-validate/"
  />
  <LinkCard
    title="preflight"
    description="Run first to confirm the dev server and browser automation are ready."
