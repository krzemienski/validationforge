...
Platform validation for HTTP APIs. Uses `curl` + `jq` to exercise real endpoints
against a real server and saves **full response bodies** — not just status
codes — as evidence.

## When to Use

Pick `api-validation` when:

- The project exposes HTTP routes (Express, Fastify, Next.js API routes,
  FastAPI, Flask, Django REST, Rails, Go/Chi, Axum, etc.).
- You need to prove CRUD flows, auth, error handling, pagination, or rate
  limiting actually behave correctly.
- You want evidence files you can diff across builds.

For end-to-end journeys that span API + UI, see
[`fullstack-validation`](https://github.com/krzemienski/validationforge/blob/main/skills/fullstack-validation/SKILL.md)
and [`e2e-validate`](/skills/e2e-validate/).
...
| Auth tokens available (optional)  | Login endpoint returns a token                            |
...
Verify the response includes expected fields (status, version, uptime, etc.).
A bare `200 OK` without body content is suspicious — document it.
...
If rate limiting exists, expect **429** status after threshold with a
`Retry-After` header.

## Evidence Standards

<Aside type="tip" title="Save the body — always">
  **GOOD:** Full JSON response body saved to file, status code captured,
  response describes what data was returned.

  **BAD:** "API returned 200" or "Request succeeded" without saving the
  response body.

  Every evidence file must contain the FULL response body — not just a status
  code.
</Aside>
...
This skill exercises a real API through the same interface real clients use.
It never bypasses authentication, never disables rate limiting, and never
injects test-only routes — it validates through the security boundaries
already in place.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Evidence examination before marking the API validation complete."
    href="/skills/gate-validation-discipline/"
  />
  <LinkCard
    title="e2e-validate"
    description="Multi-step journey orchestration that wraps api-validation."
    href="/skills/e2e-validate/"
  />
  <LinkCard
    title="preflight"
    description="Run first to confirm the API server, DB, and auth tokens are ready."
    href="/skills/preflight/"
  />
  <LinkCard
    title="fullstack-validation"
    description="Bottom-up validation order (DB → API → Frontend) for end-to-end apps."
    href="https://github.com/krzemienski/validationforge/blob/main/skills/fullstack-validation/SKILL.md"
  />
</CardGrid>
